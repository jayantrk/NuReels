...
for video in *.mp4; do
    ...
    ffmpeg -i "$video" -b:v 500k -vf scale=1080:1920 -c:v libx264 -preset fast -crf 28 -an "$output"
    ...
    while [ $(stat -c %s "$output") -gt $MAX_SIZE ]; do
        ...
        ffmpeg -i "$video" -b:v 300k -c:v -vf scale=1080:1920 libx264 -preset fast -crf 30 -an "$output"
    done
